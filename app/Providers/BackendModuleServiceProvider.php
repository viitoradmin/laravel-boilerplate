...
class BackendModuleServiceProvider extends ServiceProvider
{
    ...
    {
        $this->files = $files;
        if (is_dir(app_path(Config::get('modules.default.backend_directory')))) {
            $modules = array_map(
                'class_basename',
                $this->files->directories(app_path(Config::get('modules.default.backend_directory')))
            );
    ...
    protected function getRoutingConfig(string $module): array
    {
        ...
        $namespace = $this->app->getNamespace() . trim(
            Config::get('modules.default.backend_directory') . "\\{$module}\\" . implode('\\', explode('/', $cp)),
            '\\'
        );
    ...
    protected function registerRoute(string $module, string $path, string $namespace, string $type): void
    {
        ...
        $file = str_replace(
            ...
            '/',
            app_path(
                Config::get('modules.default.backend_directory')
                . DIRECTORY_SEPARATOR
                . ($module)
                . DIRECTORY_SEPARATOR
                . ($path)
    ...
    protected function prepareComponent(string $module, string $component, string $file = '')
    {
        ...
        $resource = rtrim(
            str_replace('//', '/', app_path(Config::get('modules.default.backend_directory') . "/{$module}/{$path}/{$file}")),
            '/'
        );
    ...
    /**
     * Register the config for a module by its name
     *
     * @param string $module
     * @return void
     */
    ...
    {
        if ($config = $this->prepareComponent($module, 'config')) {
            if (!$this->files->isEmptyDirectory($config)) {
                foreach ($this->files->allFiles($config) as $fileInformation) {
                    $this->loadConfigFrom($config, $fileInformation->getBasename('.php'));
                }
            }
        }
    }
    ...
    protected function loadConfigFrom(string $path, string $namespace): void
    {
        ...
        $this->mergeConfigFrom(
            $path.DIRECTORY_SEPARATOR.strtolower($namespace).'.php',
